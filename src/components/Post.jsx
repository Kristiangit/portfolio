...
export default function Post({title, text, colors}) {
    ...
    return (
      <div className="w-full h-screen ">
        ...
        <div className={`w-full h-screen flex flex-row items-center justify-center    bg-gradient-to-b from-${colors[0]} via-${colors[1]} via-90% to-${colors[2]}`}>
          <div className="w-1/2 flex flex-col items-center justify-around">
            <h1 className="text-4xl font-bold" >{title}</h1>

            <p>{text}</p>
          </div>
          <div className="w-1/3 h-1/3 bg-orange-600"></div>
